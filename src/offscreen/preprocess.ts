...
export async function prepareImage(url: string, manifest: ModelManifest): Promise<PreparedImage> {
  ...
  const tensor = bitmapToTensor(bitmap, manifest);
  bitmap.close();

  return { tensor, metadataSignals };
}

function bitmapToTensor(bitmap: ImageBitmap, manifest: ModelManifest): Float32Array {
